    ...
    public void checkTile(Entity entity) {
        ...
        switch (entity.direction) {
            ...
            case "left":
                ...
                // It's also good practice to check if nextLeftCol and entityTopRow/BottomRow are
                // within the bounds of mapTileNum itself. Assuming mapTileNum has dimensions like
                // gp.tileManager.maxWorldCol and gp.tileManager.maxWorldRow.
                // If they are out of mapTileNum's bounds, it's definitely a collision.
                // For this specific error, the issue is the *content* of mapTileNum.
                ...
                // Check if tileNum1 is a valid index and if the tile is solid
                boolean collisionTile1 = false;
                if (tileNum1 >= 0 && tileNum1 < gp.tileManager.tile.length) { // Check against tile array bounds
                    if (gp.tileManager.tile[tileNum1].collision) {
                        collisionTile1 = true;
                    }
                } else {
                    // tileNum1 is an invalid index (e.g., -1). Treat as collision.
                    collisionTile1 = true;
                }

                // Check if tileNum2 is a valid index and if the tile is solid
                boolean collisionTile2 = false;
                if (tileNum2 >= 0 && tileNum2 < gp.tileManager.tile.length) { // Check against tile array bounds
                    if (gp.tileManager.tile[tileNum2].collision) {
                        collisionTile2 = true;
                    }
                } else {
                    // tileNum2 is an invalid index (e.g., -1). Treat as collision.
                    collisionTile2 = true;
                }
                if (collisionTile1 || collisionTile2) {
                    entity.collisionON = true;
                }
                break;
            case "right":
